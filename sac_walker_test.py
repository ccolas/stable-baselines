...
def main(env_name, logdir, n_env, buffer_size, eval_freq, total_timesteps, nb_eval_rollouts, exp_name, batch_size):
    print("launching sac with env {}, logs in {}, n_env {} bufsize {} eval_freq {} total_timesteps {} nb eval rols {} e_name {} btchsize {}"
          .format(env_name, logdir, n_env, buffer_size, eval_freq, total_timesteps, nb_eval_rollouts, exp_name, batch_size))
    #
    # BipedalWalker-v2:
    ...
    # train_env = SubprocVecEnv([lambda: gym.make(env) for _ in range(n_env)])
    # eval_env = SubprocVecEnv([lambda: gym.make(env) for _ in range(n_env)])
    #
    # model = SAC(MlpPolicy, train_env, eval_env, verbose=1, learning_starts=1000, tensorboard_log="./sac_walker_tensorboard/",
